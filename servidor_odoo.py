...
TOOLS = {
    "tools": [
        ...
        {
            "name": "test_movimiento",
            "description": "Prueba: crea un movimiento de stock simple sin albarán.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "sku": {"type": "string", "description": "SKU del producto."},
    ...
    if method == "initialize":
        ...
        # ---- HERRAMIENTA DE RESERVA ----
            ...
            if not product:
                ...
                # 3. Crear albarán de entrega con el movimiento incluido
        ...
        # ---- HERRAMIENTA DE PRUEBA ----
        elif tool_name == "test_movimiento":
            sku = arguments.get("sku", "")
            cantidad = arguments.get("cantidad", 0)
            ...
            if product:
                move_id = models.execute_kw(ODOO_DB, uid, ODOO_PASSWORD,
                    "stock.move", "create", [{
                        "product_id": product[0]["id"],
                        "product_uom_qty": cantidad,
                        "name": product[0]["name"],
                    }])
                results = {"move_id": move_id, "status": "Movimiento creado correctamente"}
            else:
